...
int pantalla_mostrarDebug(Pantalla* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        for(i=0;i<limite;i++)
        {
            printf("[DEBUG] - %d - %s - %s - %f - %d - %d\n",array[i].idPantalla, array[i].nombre,array[i].apellido,array[i].cuitb,array[i].tipo, array[i].isEmpty);
        }
    }
    return retorno;
}
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        for(i=0;i<limite;i++)
        {
             ...
             if((array[i].cuitb)<1000)
            {
                ...
                printf("\nId:%d  Nombre: %s  apellido: %s  cuitb: %f  \n",array[i].idPantalla, array[i].nombre, array[i].apellido,array[i].cuitb);
            }
        }
...
int pantalla_altaForzada(Pantalla* array,int limite,char* nombre,char* apellido, float cuitb, int tipo)
{
    int retorno = -1;
    int i;
