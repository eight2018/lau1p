...
int cont_mostrar(Contratacion* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        {
            if(!array[i].isEmpty)
                printf("  id:%d - estado (1 activo o 2) inactivo:%d - rubro:%d - IsEmpty%d\n",array[i].id,array[i].archivo,array[i].rubro, array[i].isEmpty);
        }
    }
    return retorno;
}
        ...
        if(i >= 0)
        {
            ...
            { // la pantalla existe, idPant es valido
                if (!getValidInt("rubro?","\nNumero no valido\n",&diaAux,0,200,2))
                {
                    ...
                    {
                        if (!getValidInt("estado? 1activo   2 incactivo","\nNumero no valido\n",&archivoAux,1,2,2))
                        //if (!getValidString("estado? s  o n","Archivo no valido\n","error",archivoAux,2,2))
                        {
                            arrayC[i].rubro = diaAux;
                            arrayC[i].idPantalla = idPant;
                            arrayC[i].archivo = archivoAux;// relacion
        ...
        if (!getValidInt("rubro?","\nNumero no valido\n",&diaAux,1,2,2))
                {
                    //getStringNumeros("Cuit?",cuitAux);
                    {
                        (!getValidInt("estado? 1) activo 2)inactivo","\nNumero no valido\n",&archivoAux,1,2,2))
